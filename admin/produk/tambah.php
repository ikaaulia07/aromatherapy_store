...
if ($pdo) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($nama_produk) && $harga > 0 && $stok >= 0) {
            try {
                $stmt = $pdo->prepare("INSERT INTO produk (id_kategori, nama_produk, deskripsi, harga, stok, gambar) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id_kategori, $nama_produk, $deskripsi, $harga, $stok, $gambar_nama]);
                setFlashMessage('success', 'Produk baru berhasil ditambahkan.');
                redirect('index.php');
            } catch (\PDOException $e) {
                setFlashMessage('danger', 'Gagal menyimpan produk baru.');
            }
        } else {
    ...
    <div class="admin-content">
        <div class="admin-header">
            <div>
                ...
                <p style="color: var(--text-muted); margin-top: 5px;">Unggah detail lilin aromaterapi dan essential oil baru.</p>
            </div>
        ...
        <div class="auth-wrapper" style="margin: 0 auto; max-width: 700px; padding: 40px; border-radius: 20px;">
            <form action="tambah.php" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="nama_produk" id="nama_produk" class="form-control" placeholder="Contoh: Lavender Soy Candle 120g" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="form-control" rows="4" placeholder="Tuliskan deskripsi lengkap produk..." required></textarea>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div class="form-group">
                        <label for="harga" class="form-label">Harga (Rupiah)</label>
                        <input type="number" name="harga" id="harga" class="form-control" placeholder="85000" min="0" required>
                    </div>
                    <div class="form-group">
                        <label for="stok" class="form-label">Stok</label>
                        <input type="number" name="stok" id="stok" class="form-control" placeholder="10" min="0" required>
                    </div>
                </div>
                
                <div class="form-group">
                    <label for="gambar" class="form-label">Gambar Produk (Format JPG/PNG)</label>
                    <input type="file" name="gambar" id="gambar" class="form-control" accept="image/*" style="padding: 8px 12px;">
                </div>
                
                <button type="submit" class="btn btn-primary" style="width: 100%; height: 48px; margin-top: 10px;">Simpan Produk</button>
            </form>
        </div>
    </div>
</body>
